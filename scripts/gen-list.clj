...
(defn file-data [plugin-name]
  ...
  (let [file (io/file (str "./TypeScript/" plugin-name))
        lines (->> (read-lines file) 
                   ...
                   (map group-doc))
        [description, 
         [author version example & remaining]] (get-description lines)]
        [[:description (reduce combine-lines "" (map :unknown description))]
         [:version (:version version)]
         [:author (:author author)]
         [:example (reduce combine-lines "" (map :unknown remaining))]]
    ))

(defn get-filename [file] (.getName file))
